...
export function ActivationForm({ token }: ActivationFormProps) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    setLoading(true);
    try {
      const response = await fetch("/api/clients/activate", {
        method: "POST",
      ...
      setError("Aktivasyon sırasında hata oluştu.");
    } finally {
      setLoading(false);
    }
  }
  ...
  if (success) {
    return (
      <div className="space-y-3 text-center">
        <h2 className="text-xl font-semibold text-slate-900">Şifreniz hazır!</h2>
        <p className="text-sm text-slate-500">
          Hesabınız aktifleştirildi. Az sonra giriş sayfasına yönlendirileceksiniz.
        </p>
      </div>
    );
  }
